...
{
    public partial class ctrUserActive : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void gvEmployeeLog_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            ...
            if (e.Column.Name == "Status")
            {
                if (e.RowHandle >= 0)
                {
                    if (e.CellValue.ToString() == "Đang Hoạt Động")
                    {
        ...
        private async void loadData()
        {
            while (isWorking)
            {
                ...
                IEnumerable result = from u in user.AsEnumerable()
                                     select new
                                     {
                                         id = u.Field<string>("id"),
                                         name = u.Field<string>("name"),
                                         lastsignin = u.Field<string>("lastsignin"),
                                         status = u.Field<string>("isActive") == "1" ? "Đang hoạt động" : "Đã đăng xuất",
                                         block = u.Field<string>("isBlocked") == "1" ? true : false,
                                     };
